...
for p=1:cf
%% C-relaxation 
 new_value = g{l}(:,Cpts(1)) + feval(Phi, uv{l}(:,(end)), t{l}(1), t{l}(length(t{l})-1), app, l);
    uv{l}(:,Cpts(1)) = uv{l}(:,Cpts(1)) + omega * (new_value - uv{l}(:,Cpts(1)));
    for i=Cpts(2:end)
    %   uv{l}(:,i) = g{l}(:,i)...
      %           + feval(Phi,uv{l}(:,i-1),t{l}(i),t{l}(i-1),app,l);
...
xlabel('t'); ylabel('x');
    ...
    g{l+1}(:,1) = g{l}(:,Cpts(1)) +...
                  feval(Phi,uv{l}(:,Fpts(end)),t{l}(Cpts(end)),t{l}(Cpts(end)-1),app,l)-...
                   uv{l}(:,Cpts(1));
    for i=2:length(Cpts)
   g{l+1}(:,i) = g{l}(:,Cpts(i)) + ...
